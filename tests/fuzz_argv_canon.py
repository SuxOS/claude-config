...
# `VAR=VAL` stay in WRAPPER_EXTRA_SUFFIXES below. `-S`/`--split-string` is DELIBERATELY ABSENT from
# this table too (not just from `_hookutil.WRAPPER_VALUE_OPTS`): this same reference table encoded
# the identical wrong "flag consumes one opaque following token" mental model for `-S` that
# `_hookutil.py` did, so a generator driven by it couldn't have caught #227 either — an independent
# table is only as good as the human/agent model behind it, and this one guessed wrong in the same
# direction as the code it was meant to keep honest (#228). `-S`'s real shell-word-splitting shape
# is covered on its own axis by `env_split_string_variants()` below, generated straight from
# REAL_ARGV rather than a second hand-guessed value.
...
def env_split_string_variants():
    """(prefix_tokens, expected) pairs for env's -S/--split-string, on its own axis (#227, #228):
    unlike every flag in REFERENCE_WRAPPER_VALUE_FLAGS, `-S`'s value isn't an opaque token sitting
    in front of REAL_ARGV — it's shell-word-split and BECOMES REAL_ARGV (real env(1) behavior,
    verified against the actual binary while building this fix). So each case here embeds REAL_ARGV
    (or a trivial transform of it) inside the `-S` value directly, rather than being generated by
    `wrapper_variants()`'s "prefix, then REAL_ARGV appended untouched" pattern."""
    joined = " ".join(REAL_ARGV)
    return [
        (["env", "-S", joined], REAL_ARGV),
        (["env", "-S" + joined], REAL_ARGV),  # glued short form
        (["env", "--split-string", joined], REAL_ARGV),
        (["env", "--split-string=" + joined], REAL_ARGV),
        # a real trailing arg after the -S value must be APPENDED, not dropped
        (
            ["env", "-S", "%s extra" % REAL_ARGV[0]] + REAL_ARGV[1:],
            [REAL_ARGV[0], "extra"] + REAL_ARGV[1:],
        ),
        # double-quoting one word inside the value must not change the recovered argv
        (["env", "-S", '"%s" %s' % (REAL_ARGV[0], REAL_ARGV[1])], REAL_ARGV),
        # `\_` is env's own word-separator escape, used to avoid a literal space
        (["env", "-S", joined.replace(" ", "\\_")], REAL_ARGV),
        # a wrapper prefix embedded INSIDE the split string must still resolve through it
        (["env", "-S", "sudo -u x %s" % joined], REAL_ARGV),
    ]
...
def main():
    ...
    # #227/#228: env -S/--split-string's own axis — its value isn't a "prefix + REAL_ARGV" shape.
    for prefix_tokens, expected in env_split_string_variants():
        got = strip_prefixes(prefix_tokens)
        if got != expected:
            key = ("env-split-string", tuple(prefix_tokens))
            if key not in violations:
                violations[key] = (
                    "strip_prefixes() env -S invariant: %r should recover %r, got %r"
                    % (prefix_tokens, expected, got)
                )
