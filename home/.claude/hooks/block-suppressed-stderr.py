...
# `(?=[\s;&|]|$)` after `null` requires a word/path boundary right there — whitespace, a shell
# metacharacter (`;`/`&`/`|`), or end-of-string — so a real, longer path that merely STARTS with
# the seven characters `/dev/null` (`/dev/nullish.log`, `/dev/null-backup/errors.txt`) doesn't
# match as a prefix of the literal target (#386), mirroring the same "own word" care already taken
# for the digit above.
