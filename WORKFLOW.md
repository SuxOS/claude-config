...
- **`home/.claude/hooks/`** — the cardinal rails as code, dispatched through one
  PreToolUse entry point (`pretooluse-bash.py`). `verify-completion-claim` is built
  but off — arm it when you trust it. `hooks/README.md` is the definitive
  live/disabled list; don't re-enumerate it here, it drifts (#169).
